...
#readPiFace6.py - Derived from readPiFace5.py.  Clarifies between fps and internal framerate for camera.  In testing.
#Written by Michelle Sit

#Reads the values from the piFace and sends them to manualPic2.py to trigger
...
class callAll():
	...
	def __init__(self):
		global totalTimeEngAnswers, totalTimeAnswers, camTimeIntervalEngAnswers, camTimeIntervalAnswers, totalTime, totalTimeEng, videoTime, videoTimeEng
		global resAnswers, vidResAnswers, camResAnswers, resolutionW, resolutionH
		global FPSanswers, customNumFramesAnswers, customTimeIntervalAnswers, inputFPS, numFrames, timeInterval, internalCamFR
		global runMode, exitAnswers, exitConfirm, exitVerify, inputRun, currentMode, cmdStr, exitAns, emergencyAns
		...
		#For camera, inputFPS sets numFrames and timeInterval variables internally to achieve the desired fps.
		...
		p1.numFrames = 0
		p1.timeInterval = 0
		p1.internalCamFR = 0 #Used to set internal camera framerate (different than fps for camera)
	...
	def homeScreen(self):
		...
		if p1.currentMode == "VID":
			cad.lcd.write("{0} TT{1} VT{2}\nRes:{3}p FR:{4}".format(p1.currentMode, p1.totalTimeEng, p1.videoTimeEng, p1.resolutionH, p1.inputFPS))
#######NEED TO CHANGE THIS
		else:
...
#HELPER METHODS:
	...
	def updateCMD(self):
		if p1.fileName == "" or p1.totalTime == 0 or p1.resolutionW == 0 or p1.resolutionH == 0 or p1.inputFPS == 0 or \
	...
	def determineFPS(self):
		if p1.currentMode == "VID":
			if p1.resolutionH == "1080":
				...
				p1.FPSanswers = ["15", "30", "49"]
			elif p1.resolutionH == "480":
				p1.FPSanswers = ["1", "49", "60", "90"]
			else:
				p1.FPSanswers = ["0"]	
		elif p1.currentMode == "CAM":
			if p1.resolutionH == "1080":
				p1.FPSanswers = ["1", "2", "3", "5", "8", "Custom"]
				p1.internalCamFR = 90
			elif p1.resolutionH == "1944":
				p1.FPSanswers = ["1", "2", "3", "5", "Custom"]
				p1.internalCamFR = 50
			elif p1.resolutionH == "730":
				p1.FPSanswers = ["1", "3", "5", "15", "18", "Custom"]
				p1.internalCamFR = 50
			elif p1.resolutionH == "480":
				p1.FPSanswers = ["10", "20"]
				p1.internalCamFR = 90
...
#			p1.setCamFramesAndTime()
	...
	def manualFpsSelect(self):
		if p1.inputFPS == "Custom":
...
#			p1.inputFPS = "{0}F/{1}S".format(p1.numFrames, p1.timeInterval)
	...
	def setCamFramesAndTime(self):
		#default time interval is already set to 1 second
#		p1.timeInterval = 1
		if p1.inputFPS == "1":
			p1.numFrames = 1
		if p1.inputFPS == "2" and p1.resolutionH != "1944":
			p1.numFrames = 2
...
#		p1.inputFPS = "{0}fps".format(p1.inputFR)
	...
	def testFPS(self):
		...
		if p1.inputFPS == "Custom":
			...
			if p1.FPSanswers.count(p1.inputFPS) == 0:
				cad.lcd.clear()
				cad.lcd.write("Wrong frameRate\nTry again")
				time.sleep(1)
				p1.homeScreen()
			elif p1.FPSanswers.count(p1.inputFPS) > 0:
				pass
...
#TESTRES AND TESTFR ARE EXECUTING TWICE
	...
	def runCMD(self):
		if p1.currentMode == "VID":
			...
			".format(p1.videoTime, p1.resolutionW, p1.resolutionH, p1.totalTime, p1.inputFPS)
		elif p1.currentMode == "CAM":
...
#				p1.inputFR = 90
			...
			".format(p1.totalTime, p1.resolutionW, p1.resolutionH, p1.numFrames, p1.timeInterval, p1.internalCamFR)
