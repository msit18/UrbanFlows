...
#Reads the values from the piFace and sends them to pictureCaptureIvport.py to trigger
...
class callAll():
	...
	def init(self):
		global resAnswers, timeEngAnswers, timeAnswers, vidTimeEngAnswers, vidTimeAnswers, TFanswers, runMode, exitAnswers, exitConfirm, emergencyConfirm
		...
		timeEngAnswers = ["2sec",  "5sec", "10sec", "15sec", "20sec", "30sec", "45sec", "1M", "2M", "3M", "5M", "7M", "10M", "20M"]
		timeAnswers = ["2", "5", "10", "15", "20", "30", "45", "60", "120", "180", "300", "420", "600", "1200"] #insert forever option at some point
		...
		exitAnswers = ["MPi exit prgm", "Restart", "Shutdown", "Num file in fldr", "Clear mvFolder", "Return menu"]
	...
	def homeScreen(self):
		...
		if p1.currentMode == "VID":
			...
			cad.lcd.write("{0}: TT:{1} TF:{2}\nRes:{3}p".format(p1.currentMode, p1.inputTime, p1.inputFR, p1.resolutionH))
...
#		try:
		...
		#S5: SHUTDOWN BUTTON
			...
			p1.S5()
#		except:
#			os.system("python flash.py error")
#			end_barrier.wait()
...
#S1-5 PROGRAMMED COMMANDS:
	...
	def S3(self):
		cad.lcd.clear()
		#Takes input
		question = LCDQuestion(question="Set TotalTime", answers=timeEngAnswers)
		answer_index = question.ask()
		cad.lcd.clear()
		#User feedback
		if p1.currentMode == "VID":
			#Takes the first time input as total running time 
			#note: timeEngAnswers is for usability, stored number is array answer from timeAnswers
			p1.inputTime = timeAnswers[answer_index]
			cad.lcd.write("Total Time\n{0}".format(timeEngAnswers[answer_index]))
			time.sleep(1)
			cad.lcd.clear()
...
#				end_barrier.wait()
	...
	def updateCMD(self):
		if p1.fileName == "" or p1.inputTime == 0 or p1.resolutionW == 0 or p1.resolutionH == 0 or p1.inputFR == 0:
			...
			p1.testFR()
			#p1.cmdStr = "sudo python {0} {1} {2} {3} {4} {5}".format(p1.fileName, p1.inputTime, p1.resolutionW, p1.resolutionH, p1.inputFR, p1.videoTime)
	...
	def testFR(self):
		...
		if p1.TFanswers.count(p1.inputFR) == 0:
			...
			p1.cmdStr = "sudo python {0} {1} {2} {3} {4} {5}".format(p1.fileName, p1.inputFR, p1.resolutionW, p1.resolutionH, p1.inputTime, p1.videoTime)
