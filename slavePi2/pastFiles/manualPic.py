		...
		with picamera.PiCamera() as camera:
			...
			for k in range (timeFrames):
				camera.capture_sequence([
					datetime.datetime.now().strftime ('%d-%m-%Y-%H_%M_%S_%f') + '_ST' + str(sys.argv[1]) + '_RF' + str(sys.argv[2]) + '_FR' + str(sys.argv[4]) + '.jpg'
					for i in range(frames)
					], use_video_port=True)
				#print ('Thread 1 Captured %d frames' %(frames)
				self.queue.put('beginT2')
		finish =  time.time()
		print ('Captured %d frames at %2ffps in %2secs' %(timeFrames*frames, ((timeFrames*frames)/(finish-start)), (finish-start) ))
